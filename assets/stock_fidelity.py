...
class StockFidelityNetBenefits(StockBrokerBase):
    ...
    def _establish_session(self, username, password):
        s = requests.Session()
        s.get(self.LOGIN_URL, headers=self.HEADERS)
        ...
        sensor_response = s.post(self.SENSOR_DATA_URL, json=post_data, headers=self.HEADERS)
        sensor_response_data = json.loads(sensor_response.text)
        assert sensor_response_data.get("success")

        s.get(self.PRE_LOGIN_URL, headers=self.HEADERS)
        s.get(self.LOGIN_INIT_URL, headers=self.HEADERS)
        ...
        login_response = s.post(self.LOGIN_POST_URL, data=post_data, headers=self.HEADERS)
